...
const pic = (i: number) => IMG + imgs[i % imgs.length];
...
const tabs = [
  {
    key: 'mini',
    label: 'Mini Game',
    games: mk(['Mega Fortune', 'Starburst', 'Limbo', 'Mines', 'Plinko', 'Dice', 'Tower', 'Keno', 'Hilo', 'Wheel', 'Crash', 'Coin Flip', 'Rocket', 'Caves', 'Video Poker', 'Scratch Card']),
  },
  {
    key: 'slot',
    label: 'Slot Game',
    games: mk(['Gates of Olympus', 'Sweet Bonanza', 'Book of Dead', 'Starburst', 'Wolf Gold', 'Mega Moolah', 'Gonzo Quest', 'Dead or Alive', 'Sugar Rush', 'Big Bass', 'Money Train', 'Wild West Gold']),
  },
  {
    key: 'live',
    label: 'Live Game',
    games: mk(['Lightning Roulette', 'Crazy Time', 'Mega Wheel', 'Baccarat', 'Dragon Tiger', 'Monopoly Live', 'Blackjack VIP', 'Sic Bo', 'Dream Catcher', 'Speed Roulette', 'Football Studio', 'Andar Bahar']),
  },
];
  ...
  <section class="py-12 bg-[#1a2128]">
    <div class="container mx-auto px-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div class="grid grid-cols-3 md:flex md:items-center md:gap-8">
            ...
            >
              ...
              <div v-show="active === 'mini'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-700">
                <div class="h-full bg-gradient-to-r from-[#CBE8E4] to-[#98E7D2]"></div>
              </div>
            </button>
            <button
              class="pb-2 transition-colors relative flex flex-col md:flex-row items-center justify-center md:justify-start gap-1 md:gap-2"
            ...
            >
              ...
              <div v-show="active === 'slot'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-700">
                <div class="h-full bg-gradient-to-r from-[#CBE8E4] to-[#98E7D2]"></div>
              </div>
            </button>
            <button
            ...
            >
              ...
              <div v-show="active === 'live'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-700">
                <div class="h-full bg-gradient-to-r from-[#CBE8E4] to-[#98E7D2]"></div>
              </div>
            </button>
          </div>
          <div class="flex items-center justify-end gap-2">
            <a class="text-gray-400 hover:text-white text-xs px-3 py-1.5 border border-gray-700 rounded transition-colors" href="/mini-games">Show all</a>
      ...
      <div class="overflow-hidden">
        <div :key="active" class="flex overflow-x-auto gap-3 snap-x snap-mandatory scrollbar-hide pb-4 animate-slideIn">
          <div v-for="g in games" :key="g.title" class="flex-shrink-0 w-28 md:w-32 snap-start cursor-pointer group">
            ...
            <h3 class="text-white text-xs md:text-sm text-center mt-2 truncate">{{ g.title }}</h3>
          </div>
        </div>
      </div>
    </div>
