...
function csIcon(svg) {
  return `<div style="width:44px;height:44px;border-radius:12px;background:rgba(96,140,255,.12);border:1px solid rgba(96,140,255,.35);display:flex;align-items:center;justify-content:center;color:${CS_BLUE};flex-shrink:0">${svg}</div>`;
}
...
function csRow(title, sub, icon, href) {
  return `<button data-cs-href="${href}" style="display:flex;align-items:center;gap:14px;width:100%;text-align:left;padding:16px;margin-bottom:12px;border-radius:12px;border:1px solid #2a3550;background:#121a2b;cursor:pointer;transition:all .15s">
      ${csIcon(icon)}
...
function openSupportModal() {
  ...
  o.innerHTML = `<div style="background:#0d1422;border:1px solid #1e2740;border-radius:18px;max-width:440px;width:100%;box-shadow:0 24px 70px rgba(0,0,0,.6)">
      <div style="display:flex;align-items:center;gap:12px;padding:18px 20px;border-bottom:1px solid #1a2335">
        ${csIcon(S_HEADSET)}
        <h3 style="flex:1;color:#fff;font-size:18px;font-weight:700;margin:0">Customer Service</h3>
        <button data-close style="width:34px;height:34px;border-radius:50%;border:1px solid #2a3550;background:none;color:#9ca3af;cursor:pointer;font-size:18px;line-height:1;display:flex;align-items:center;justify-content:center">×</button>
      </div>
